...
질문1 - alu에서 처리된 정보가 바로 메인메모리에 저장되면 안될까?
질문2 - 명령어에 굳이 레지스터 주소를 명시해야 하는가? 최종목적지(즉 메인메모리 주소)를 명시 하면 좋지 않을까?
>CISK의 경우에는 가능하다.
...
(레지스터의 관점에서..)
LOAD - 메인메모리에 있는 정보를 레지스터로 끌어오는 명령어
STORE - 레지스터에 있는 정보를 메인메모리에 반환 하는 명령어
...
모든정보는 레지스터를 통한다는 사실아래(RISK이므로), 어떻게 하면 이정보들을
레지스터를 통하게 명령어 구성을 할수 있을까?
